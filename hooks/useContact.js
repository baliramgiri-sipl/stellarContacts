...
import { UPDATE_CONTACT_PAGE_VIEW_DATA, UPDATE_INBOX_DATA } from '@/redux/contactReducer/contactReducer'
...
const useContact = () => {
    ...
    const { mutateAsync, isLoading: isAllLoading } = useMutation(async (data) => {
        ...
        onSuccess({ data, metaData }) {
            dispatch({ type: UPDATE_CONTACT_PAGE_VIEW_DATA, payload: { ...contactPaginationData, page: metaData?.currentPage, metaData } })
            if (isAll === 0) {
